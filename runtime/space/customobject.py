...
class CustomObject(Object):
    ...
    def getattr(self, name):
        custom_interface = jit.promote(self.custom_interface)
        method = custom_interface.lookup_method(u"+getattr")
        if method is None:
    ...
    def getattr_direct(self, name):
        ...
        if index != -1:
            ...
            custom_interface = jit.promote(self.custom_interface)
            method = custom_interface.lookup_method(name)
            if method is None:
...
def CustomObject_instantiate(interface, argv):
    obj = CustomObject(interface)
    method = interface.lookup_method(u"+init")
    if method is not None:
        method.call([obj] + argv)
    return obj
CustomObject.interface.instantiate = CustomObject_instantiate
